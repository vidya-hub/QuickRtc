...
This directory contains multiple scripts to refresh and rebuild all Simple MediaSoup dependencies. These scripts are designed to handle the complete build pipeline for the monorepo structure.
...
2. **Build Dependencies in Order**
   ...
   - Builds `simple_ms_types` (shared TypeScript types)
   - Builds `simple_ms_server` (server library)
   - Builds `simple_ms_client` (client library)
...
3. **Directory Issues**: Run the script from the `simple_ms_example` directory
...
```bash
# From project root
cd simple_ms_types && npm install && npm run build && cd ..
cd simple_ms_server && npm install && npm run build && cd ..
cd simple_ms_client && npm install && npm run build && cd ..
cd simple_ms_example && npm install && npm run build:example
```
