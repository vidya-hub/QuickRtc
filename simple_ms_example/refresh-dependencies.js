...
/**
 * Simple MediaSoup - Refresh Dependencies Script (Node.js)
...
async function main() {
  colorLog("🔄 Simple MediaSoup - Refreshing Dependencies", "blue");
  colorLog("=============================================", "blue");

  const projectRoot = path.resolve(__dirname, "..");
  const exampleDir = path.resolve(projectRoot, "simple_ms_example");
  ...
  try {
    ...
    printStep("Building simple_ms_types");
    await runCommand("npm install", path.join(projectRoot, "simple_ms_types"));
    await runCommand(
      "npm run build",
      path.join(projectRoot, "simple_ms_types")
    );
    printSuccess("simple_ms_types built successfully");

    printStep("Building simple_ms_server");
    await runCommand("npm install", path.join(projectRoot, "simple_ms_server"));
    await runCommand(
      "npm run build",
      path.join(projectRoot, "simple_ms_server")
    );
    printSuccess("simple_ms_server built successfully");

    printStep("Building simple_ms_client");
    await runCommand("npm install", path.join(projectRoot, "simple_ms_client"));
    await runCommand(
      "npm run build",
      path.join(projectRoot, "simple_ms_client")
    );
    printSuccess("simple_ms_client built successfully");
    ...
    const publicClientDir = path.join(exampleDir, "public", "simple_ms_client");
    const clientDistDir = path.join(publicClientDir, "dist");
    const sourceDistDir = path.join(projectRoot, "simple_ms_client", "dist");
    ...
    console.log("  • Built simple_ms_types");
    console.log("  • Built simple_ms_server");
    console.log("  • Built simple_ms_client");
