...
const getLocalIp = () => {
  ...
  if (!ifaces) {
    console.warn("os.networkInterfaces() returned undefined, using localhost");
  ...
  for (const ifname of Object.keys(ifaces)) {
    ...
    for (const iface of ifaceList) {
      ...
      // Set the local ip to the first IPv4 address found and exit the function
...
class MediaSoupExpressServer {
  ...
  private socketServer?: SocketIOServer<
    ...
    ClientToServerEvents
  >;
  private mediaServer?: QuickRTCServer;
  ...
  private createMediaServer(
    ...
    const mediaServerConfig: QuickRTCServerConfig = {
      // Inject our servers instead of letting QuickRTCServer create them
      httpServer,
      socketServer,
      mediasoup: {
        workerSettings: {
    ...
    const mediaServer = new QuickRTCServer(mediaServerConfig);

    this.setupMediaServerEvents(mediaServer);
    return mediaServer;
  }

  private setupMediaServerEvents(mediaServer: QuickRTCServer): void {
    // Server events
    mediaServer.on(
      "serverStarted",
      (event: CustomEvent<QuickRTCServerEvents["serverStarted"]>) => {
    ...
    mediaServer.on(
      "serverError",
      (event: CustomEvent<QuickRTCServerEvents["serverError"]>) => {
        const timestamp = new Date().toISOString();
        console.error(
    ...
    mediaServer.on(
      "clientConnected",
      (event: CustomEvent<QuickRTCServerEvents["clientConnected"]>) => {
        const timestamp = new Date().toISOString();
        console.log(
    ...
    mediaServer.on(
      "clientDisconnected",
      (event: CustomEvent<QuickRTCServerEvents["clientDisconnected"]>) => {
        const timestamp = new Date().toISOString();
        console.log(
    ...
    mediaServer.on(
      "participantLeft",
      (event: CustomEvent<QuickRTCServerEvents["participantLeft"]>) => {
        const timestamp = new Date().toISOString();
        const { participant } = event.detail;
        console.log(
    ...
    mediaServer.on(
      "conferenceCreated",
      (event: CustomEvent<QuickRTCServerEvents["conferenceCreated"]>) => {
        const timestamp = new Date().toISOString();
    ...
    mediaServer.on(
      "conferenceDestroyed",
      (event: CustomEvent<QuickRTCServerEvents["conferenceDestroyed"]>) => {
        const timestamp = new Date().toISOString();
        console.log(
    ...
    mediaServer.on(
      "producerCreated",
      (event: CustomEvent<QuickRTCServerEvents["producerCreated"]>) => {
        const timestamp = new Date().toISOString();
    ...
    mediaServer.on(
      "producerClosed",
      (event: CustomEvent<QuickRTCServerEvents["producerClosed"]>) => {
        const timestamp = new Date().toISOString();
    ...
    mediaServer.on(
      "consumerCreated",
      (event: CustomEvent<QuickRTCServerEvents["consumerCreated"]>) => {
        const timestamp = new Date().toISOString();
    ...
    mediaServer.on(
      "consumerClosed",
      (event: CustomEvent<QuickRTCServerEvents["consumerClosed"]>) => {
        const timestamp = new Date().toISOString();
    ...
    mediaServer.on(
      "audioMuted",
      (event: CustomEvent<QuickRTCServerEvents["audioMuted"]>) => {
        const timestamp = new Date().toISOString();
    ...
    mediaServer.on(
      "audioUnmuted",
      (event: CustomEvent<QuickRTCServerEvents["audioUnmuted"]>) => {
        const timestamp = new Date().toISOString();
        const { participantId, conferenceId } = event.detail;
    ...
    mediaServer.on(
      "videoMuted",
      (event: CustomEvent<QuickRTCServerEvents["videoMuted"]>) => {
        const timestamp = new Date().toISOString();
        const { participantId, conferenceId } = event.detail;
    ...
    mediaServer.on(
      "videoUnmuted",
      (event: CustomEvent<QuickRTCServerEvents["videoUnmuted"]>) => {
        const timestamp = new Date().toISOString();
        const { participantId, conferenceId } = event.detail;
